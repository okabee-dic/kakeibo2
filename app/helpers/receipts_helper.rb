module ReceiptsHelper
  ...
  def get_result_on_succeed(data, input)
    ...
    result[:html] = render_to_string :partial => "books/receipt_row", :locals => { receipt: data, type: result[:type] }
  ...
  def get_params_to_hash
    input = {}
    input[:pay_date] = params[:pay_date].tr("０-９", "0-9").to_i.abs
    input[:store_name] = params[:store]
    input[:price] = params[:price].tr("０-９", "0-9").to_i.abs
